...
6. Далее будете шаг за шагом, объявлять и определять состояние за состоянием. Вам понадобятся файлы mboard.h, mtools.h и mdisplay/.h в которыx объявлены все утилиты, составляющие ваш арсенал разработчика. А ниже представлен пример реализации одного из состояний режима заряда с подробными комментариями.
