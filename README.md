...
#### <p align="right">2023.04.19 добавлен драйвер SAMD21
...
пид-регулятор отключает преобразователь, если имеется избыток мощности ... Но что характерно - ШИМ при этом 50/50, что соответствует отдаче половины максимальной мощности, ток в режиме CC и напряжение в режиме CV поддерживаются в заданных параметрах плюс-минус 2 миллиампера и 2 милливольта:
![](https://github.com/olmoro/MKlon4.0/blob/main/documents/samd/docs/img/50_50.png)

при этом через 5 миллисекунд генерация возобновляется:
![](https://github.com/olmoro/MKlon4.0/blob/main/documents/samd/docs/img/off_5ms.png)

Словом, ведет себя как регулятор 4016, снижая частоту ШИМ в такой же ситуации с нагрузкой. Разница в том, что здесь как бы два ШИМа, но дроссель работает на родной частоте, и это, думаю, очень хорошо. Чесслово ... это не я, так получилось, буду разбираться. Но что бесспорно - это возможно только в конфигурации "весь реал-тайм на одном кристалле", с чем себя и вас поздравляю.
