...
#### <p align="right">редакция от 12 апреля 2023 года
...
![](https://github.com/olmoro/MKlon4.0/blob/main/documents/full/img/full_1.png) Вентилятор 60х60 выглядит монстром - ну нет у меня модели 50х50!
...
3. [FSM](#fsm)
3. [Первый проект](#first_project)
4. [Полезные ссылки](#useful_links)
5. [About Me](#about)

***
## <p align="center"> Как это работает.<a name="mklon"></a>
...
***
MKlon4.0. Как это работает. Межпроцессорное соединение. 
...
***
MKlon4.0. Как это работает. Межпроцессорный обмен.
...
***
MKlon4.0. Как это работает. Измерители.
...
***
MKlon4.0. Как это работает. Вычисление среднего.
...
***
MKlon4.0. Как это работает. Схема разряда.
...
***
MKlon4.0. Как это работает. Подключение заряжаемой батареи.
...
***
MKlon4.0. Как это работает. Питание микроконтроллеров.
...
***
MKlon4.0. Как это работает. Силовой DC/DC преобразователь.
...
***
MKlon4.0. Как это работает. Подключение вентилятора.
...
***
MKlon4.0. Как это работает. Программный аспект.
...
Реализацию состояния рассмотрим позже, а пока - основа проекта - базовый класс MState, отвечающий за переход между состояниями. Не приходилось использовать виртуальные функции? Мне тоже. Если что-то напутал пусть меня профи поправят.
...
***
## <p align="center">Технические характеристики MKlon4.0<a name="specifications"></a>
...
![Заряд CC/CV](https://github.com/olmoro/MKlon4.0/blob/main/documents/full/img/CC_CV_fsm.jpg)
...
1. Вооружившись решимостью запрограммировать проект заряда имени себя любимого, которого ни у кого нет - придумаем ему короткое и звучное название.

2. Найдите в папке modes файлы templatefsm.h и templatefsm.cpp с примером реализации режима и скопируйте в ту же папку но под названием вашего режима, произведя обязательные изменения в следующих стоках:
...
3. Откройте файл mdispatcher.h и в списке режимов подберите новому режиму достойное место, исключая первое и последнее - в примере ниже это там, где пустая строка. При переборе режимов кнопками он будет вызываться после TEMPLATE или CCCV в зависимости от того, в каком направлении листается список режимов. Не забудьте поставить запятую.
...
```
и введите аналогичную для вашего режима - это будет указание диспечеру осуществлять запуск именно вашего режима.
...
```c++
    ...
    break;
```
5. Компилируем, загружаем, находим наш режим, запускаем его и проверяем. СТОП! Надо бы удостовериться, что это не TEMPLATE, а именно наш. Открываем файл .cpp нашего режима и в строке
```c++
