...
```
4. Откройте файл mdispatcher.cpp и откорректируйте строку с указателе пути к файлу вашего режима
```c++
#include "modes/templatefsm.h"
```
...
```
меняем цвет светодиода с белого на какой-нибудь другой и вуаля - пусть это ещё и не проект, но видно, что именно мы виляем хвостом, а не наоборот.
